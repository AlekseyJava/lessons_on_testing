...
int OVERRIDED_lr_end_transaction (char* transaction_name,int status) {
	...
	if (status==2){
		tr_status=lr_get_transaction_status(transaction_name);
	}else{
		tr_status=status;
	}
	...
	if (Is_influx_enabled==0){
		if(tr_status != 0){
			lr_exit(LR_EXIT_ITERATION_AND_CONTINUE,tr_status);
		}
		return lr_end_transaction(transaction_name,status);
	}
	lr_start_transaction("[Service]_Influx");
	...
	tr_duration=1000*(lr_get_transaction_duration(transaction_name)-lr_get_transaction_think_time(transaction_name));
	tr_thinktime=1000*lr_get_transaction_think_time(transaction_name);
	tr_wastedtime=1000*lr_get_transaction_wasted_time(transaction_name);
	
	//End transaction using standart LR function
	tmp_res=lr_end_transaction(transaction_name,status);
	...
	tmp_TName=InfluxEscapeSymbols(transaction_name);
	...
	web_custom_request("try_write", 						
		...
		LAST);	
	lr_end_transaction("[Service]_Influx", LR_AUTO);
	if(tr_status != 0){
		lr_exit(LR_EXIT_ITERATION_AND_CONTINUE,tr_status);
	}
	return tmp_res;
...
char* lr_get_current_transaction_name(){
	return lr_eval_string("{lr_currentTransactionName}");
}

void OVERRIDED_lr_start_transaction(PROTO(LPCSTR transaction_name)){
	lr_continue_on_error(TRUE);
	lr_start_transaction(transaction_name);
	lr_save_string(transaction_name,"lr_currentTransactionName");
}

void OVERRIDED_lr_exit(int exit_option, int exit_status){
	//lr_end_transaction(lr_get_current_transaction_name(), exit_status);
	lr_set_transaction_status(LR_FAIL);
	lr_exit(exit_option, exit_status);
}
...
#define lr_start_transaction(transaction_name) OVERRIDED_lr_start_transaction(transaction_name)
#define lr_exit(exit_option,exit_status) OVERRIDED_lr_exit(exit_option,exit_status)